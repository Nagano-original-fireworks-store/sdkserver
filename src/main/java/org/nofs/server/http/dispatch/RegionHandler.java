...
import org.nofs.GameConstants;
import org.nofs.config.ConfigContainer;
import org.nofs.sdkserver;
import static org.nofs.config.ConfigContainer.Server.*;
import org.nofs.sdkserver.ServerRunMode;
import org.nofs.net.proto.QueryRegionListHttpRspOuterClass.QueryRegionListHttpRsp;
import org.nofs.net.proto.QueryCurrRegionHttpRspOuterClass.QueryCurrRegionHttpRsp;
import org.nofs.net.proto.RegionSimpleInfoOuterClass.RegionSimpleInfo;
import org.nofs.net.proto.RegionInfoOuterClass.RegionInfo;
import org.nofs.net.proto.RetcodeOuterClass.Retcode;
import org.nofs.net.proto.StopServerInfoOuterClass.StopServerInfo;
import org.nofs.server.event.dispatch.QueryAllRegionsEvent;
import org.nofs.server.event.dispatch.QueryCurrentRegionEvent;
...
import static org.nofs.config.Configuration.*;
import static org.nofs.utils.Language.translate;
...
public final class RegionHandler implements Router {
    ...
    public RegionHandler() {
        try { // Read & initialize region data.
            ...
            org.nofs.sdkserver.getLogger().error("Failed to initialize region data.", exception);
        }
    }
    ...
    private void initialize() {
        ...
        if (runMode != ServerRunMode.HYBRID && configuredRegions.size() == 0) {
            sdkserver.getLogger().error("[Dispatch] There are no game servers available. Exiting due to unplayable state.");
            System.exit(1);
        } else if (configuredRegions.size() == 0)
            configuredRegions.add(new Region("os_usa", defaultName.defaultName,
                    lr(game.Ip, game.accessAddress),
                    lr(game.accessPort, game.Port)));

        configuredRegions.forEach(region -> {
            if (usedNames.contains(region.Name)) {
                sdkserver.getLogger().error("Region name already in use.");
                return;
            }
            ...
            var regionInfo = RegionInfo.newBuilder()
                    .setGateserverIp(game.Ip).setGateserverPort(game.Port)
    ...
    private static void queryRegionList(Context ctx) {
        ...
        Logger logger =  org.nofs.sdkserver.getLogger();
...
//                
    ...
    private static void queryCurrentRegion(Context ctx) {
        ...
        if (versionMajor >= 3 || (versionMajor == 2 && versionMinor == 7 && versionFix >= 50) || (versionMajor == 2 && versionMinor == 8)) {
            try {
                ...
                if (!clientVersion.equals(GameConstants.VERSION)) { // Reject clients when there is a version mismatch
                    ...
                    org.nofs.sdkserver.getLogger().info(String.format("Connection denied for %s due to %s", ctx.ip(), updateClient ? "outdated client!" : "outdated server!"));
                ...
                org.nofs.sdkserver.getLogger().error("An error occurred while handling query_cur_region.", e);
            }
        }
        else {
            // Invoke event.
        ...
        // Log to console.
        org.nofs.sdkserver.getLogger().info(String.format("Client %s request: query_cur_region/%s", ctx.ip(), regionName));
    }
